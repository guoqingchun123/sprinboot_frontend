        ...
        <bv-form :model="item" :rules="rules">
          <el-row :gutter="$CONST.row.gutter">
            ...
            <el-col v-bind="$CONST.col.layout3" style="text-align: center">
              <bv-button type="danger" icon="el-icon-finished" @click="saveRegionData">保存</bv-button>
            </el-col>
          </el-row>
        </bv-form>
      </el-col>
...
<script>
  ...
  import {uploadFile} from '@/api/basic'

  var region = JSON.parse(sessionStorage.getItem("currRegion"));
  ...
  export default {
    ...
    mounted() {
      ...
      aMapJSAPILoader.load().then(AMap => {
        const regionMap = new AMap.Map('mapContainer', {
            center: [region.x, region.y], //初始地图中心点
    ...
    methods: {
      //自定义上传接口
      uploadImg(item) {
        let isLt = item.file.size / 1024 / 1024 < 20;
        if (!isLt) {
