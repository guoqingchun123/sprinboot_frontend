<template>
  <div class="app-container">
    ...
    <bv-row layout="table-search">
      <bv-col>
        <bv-row v-show="isFuzzy" layout="table-fuzzy">
          <bv-form ref="fuzzy">
            <bv-col>
        ...
        <bv-row v-show="!isFuzzy" layout="table-filter">
      ...
      <bv-col class="search-switch">
        <el-tooltip :content="isFuzzy ? '点击切换精确查询' : '点击切换模糊查询'">
          <el-switch v-model="isFuzzy" @change="onFuzzyChange" />
        </el-tooltip>
      </bv-col>
    </bv-row>
    <el-table
    ...
    >
      ...
      <el-table-column prop="empId" label="员工代码" sortable />
    ...
    <bv-dialog title="查看详情" :visible.sync="dialogTableVisible">
      <bv-row layout="table-search">
        <bv-col>
          <bv-row layout="table-fuzzy">
            <bv-form>
              <bv-col>
                <el-form-item prop="dialogFuzzy">
                  <el-input v-model="dialogFuzzy" placeholder="请输入查询条件" class="input-with-select" clearable>
                    <bv-button slot="append" view="search" @click="fetchDialogData" />
                  </el-input>
                </el-form-item>
              </bv-col>
            </bv-form>
